...
logger = logging.getLogger("Skills.Cyclical")
...
_USER_PROMPT = (
    ...
    "\n请基于以上信息与下方财务/分析指标，给出详细的周期股分析报告。"
)
...
def run(state: FinancialState, llm) -> Dict[str, Any]:
    """周期股分析 skill；返回 state delta。

    字段语义与原 `create_cyclical_analysis(llm)` 节点保持一致：
    - 成功：`{"cyclical_analysis": <dict>}`
    - 失败：`{"cyclical_analysis": None, "error_msg": "..."}`
    """
    ...
    if not company_name or not report_year:
        ...
        return {"cyclical_analysis": None, "error_msg": "缺少公司信息"}
    ...
    try:
        ...
        # 3. 获取市场估值数据（PE、PB、股价）
        market_data = get_stock_market_data(stock_code) if stock_code else {}
        ...
        save_skill_result("cyclical", state, result)

        return {"cyclical_analysis": result}

    except Exception as e:
        logger.error("周期股分析失败: %s", e)
        return {
            "cyclical_analysis": None,
            "error_msg": f"周期股分析失败: {e}",
        }
